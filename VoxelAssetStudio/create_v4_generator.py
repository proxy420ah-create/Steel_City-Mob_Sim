...
# skeleton_generator_actor_v4.py — 10-joint simplified actor skeleton
#
# Reduced from v2's 14 joints to 10 joints:
#   - Removed: mid_spine, chest, left_ankle, right_ankle
#   - Spine: single bone pelvis → neck (was spine_lower + spine_upper + neck)
#   - Feet: collider-only (no ankle joint, rigidly attached to shin)
#   - Collar bones: neck → shoulder (was chest → shoulder)
#
# Grid: 16 x 20 x 8  (same as v1/v2 — voxel layout unchanged)
...
def generate_actor_skeleton_v4(grid_size: Tuple[int, int, int] = (17, 20, 8)) -> Tuple[np.ndarray, Dict]:
    """Generate a 10-joint actor skeleton."""
    ...
    add_bone('spine', (cx, pelvis_y, cz), (cx, neck_joint_y, cz),
             pelvis, neck, role='spine', chain='spine',
...
if __name__ == "__main__":
    v, s = generate_actor_skeleton_v4()
...
import os
out_path = os.path.join(os.path.dirname(__file__), 'skeleton_generator_actor_v4.py')
